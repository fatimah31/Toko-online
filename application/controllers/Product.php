...
class Product extends MY_Controller {
    ...
    public function __construct() //validasi tuk admin apakah user atau admin
    {
        parent::__construct();
    ...
    public function index($page = null)
    {
        ...
        $data['content'] = $this->product->select( //menggunakan fungsi join
            [
                'product.id', 'product.title AS product_title' , 'product.image', // AS sbg aliasis krn ada nama yg sama, product image dan title
        ...
        $data['pagination'] = $this->product->makePagination(
            base_url('product'), 2 /*nilai dr halaman*/, $data['total_rows'] //total yg dibagi tiap hal
        );
    ...
    public function create()
    {
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] !== '') {
            $imageName = url_title($input->title, '-', true) . '-' . data('YmdHis');
        ...
        if (!$this->product->validate()) {
            ...
            $this->view($data);
            return; //proses berhenti pd memuat hal saja
        }
    ...
    public function edit($id) //membutuhkan parameter $id tuk memilih produk yg akan di edit
    {
        $data['content'] = $this->product->where('id', $id)->first(); //first tuk mendptkan 1 baris data
