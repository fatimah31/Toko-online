...
class Shop extends MY_Controller 
{
    ...
    public function sortby($sort, $page = null) //$sort tuk menyimpan data harga
    {
        $data['title'] = 'Belanja';
        $data['content'] = $this->shop->select( 
            [
        ...
        $data['pagination'] = $this->shop->makePagination(
            base_url("shop/sortby/$sort"), 4 /*nilai dr halaman*/, $data['total_rows'] //total yg dibagi tiap hal
        );
        $data['page'] = 'pages/home/index';
    ...
    public function category($category, $page = null)
    {
        ...
        ->orderBy('product.price', $category)
        ->paginate($page)
        ->get();
        $data['total_rows'] = $this->shop->where('product.is_available', 1)->join('category')->count();
        $data['pagination'] = $this->shop->makePagination(
            base_url("shop/category/$category"), 4 /*nilai dr halaman*/, $data['total_rows'] //total yg dibagi tiap hal
        );
